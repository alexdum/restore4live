...
colintYlGnBu <- colorRampPalette(brewer.pal(9, "YlGnBu"), interpolate = "linear")
colintRdBu <- colorRampPalette(brewer.pal(11, "RdBu"), interpolate = "linear")
...
map_cols_cmip_fun <- function(indic = NA, type = NA, domain = NA) {
  ...
  if (indic %in% c("rdis")) {
    if (type %in% "climate") {
      # Logarithmic scale for river discharge (starting from 1 m³/s up to 10000)
      df.col <- data.frame(
        cols = c(colintYlGnBu(9), "#000000"),
        vals = c(1, 10, 50, 100, 250, 500, 1000, 2500, 5000, 10000)
      )
