...
# Filter out rdis (River Discharge) from climate/SSP parameters
aoi_params_df <- aoi_params_df[aoi_params_df$param != "rdis", ]
aoi_scenarios <- c("ssp1", "ssp2", "ssp3", "ssp5")

# Hydrological parameters and scenarios (RCPs)
hydro_params_df <- data.frame(
  param = "rdis",
  name = "River Discharge (Mean)",
  stringsAsFactors = FALSE
)
hydro_scenarios <- c("rcp_2_6", "rcp_4_5", "rcp_8_5")
...
hydro_plots_data <- reactiveVal(NULL)
...
output$aoi_plots_grid <- renderUI({
  ...
  # Create UI sections for hydrological parameters (RCP scenarios)
  hydro_sections <- lapply(seq_len(nrow(hydro_params_df)), function(i) {
    param_code <- hydro_params_df$param[i]
    param_name <- hydro_params_df$name[i]

    # Create plot cards for 3 RCP scenarios
    hydro_plot_outputs <- lapply(hydro_scenarios, function(scen_code) {
      output_id <- paste("hydro_aoi_plot", param_code, scen_code, sep = "_")
      # Format RCP label nicely (rcp_2_6 -> RCP 2.6)
      scen_label <- gsub("_", " ", toupper(scen_code))
      scen_label <- gsub("RCP ", "RCP ", scen_label)
      card(
        card_header(paste("Scenario:", scen_label)),
        highchartOutput(output_id, height = "300px")
      )
    })

    # Anomaly plots for hydro
    hydro_anomaly_outputs <- lapply(hydro_scenarios, function(scen_code) {
      output_id <- paste("hydro_aoi_anomaly_plot", param_code, scen_code, sep = "_")
      scen_label <- gsub("_", " ", toupper(scen_code))
      scen_label <- gsub("RCP ", "RCP ", scen_label)
      card(
        card_header(paste("Anomaly vs 1971-2005 - Scenario:", scen_label)),
        highchartOutput(output_id, height = "300px")
      )
    })

    # Summary table for hydro
    hydro_summary_table <- tableOutput(paste("hydro_summary_table", param_code, sep = "_"))

    tagList(
      h5(param_name, style = "text-align: left; margin-top: 25px; margin-bottom: 15px;"),
      h6("Absolute values", style = "text-align: left; margin-top: 15px; margin-bottom: 15px;"),
      layout_columns(
        col_widths = c(4, 4, 4), # 3 columns for 3 RCP scenarios
        !!!hydro_plot_outputs
      ),
      h6("Anomaly values (vs 1971-2005)", style = "text-align: left; margin-top: 15px; margin-bottom: 15px;"),
      layout_columns(
        col_widths = c(4, 4, 4),
        !!!hydro_anomaly_outputs
      ),
      h6("Summary table", style = "text-align: left; margin-top: 25px; margin-bottom: 15px;"),
      hydro_summary_table,
  ...
  # Combine climate and hydrological sections into a single UI
  tagList(
    h4("Climate Scenario (SSP)", style = "text-align: left; margin-top: 10px; margin-bottom: 20px; color: #2c3e50;"),
    parameter_sections,
    h4("Hydrological Scenario (RCP)", style = "text-align: left; margin-top: 30px; margin-bottom: 20px; color: #2c3e50;"),
    hydro_sections
  )
})
...
observeEvent(list(input$aoi_selection_method, input$aoi_area, drawn_polygon_sf()),
  {
    ...
    withProgress(message = "Generating graphs and tables", value = 0, {
      ...
      # --- 5. HYDROLOGICAL DATA GENERATION ---
      hydro_data_list <- list()
      n_hydro_params <- nrow(hydro_params_df)
      n_hydro_scenarios <- length(hydro_scenarios)

      for (i in seq_len(n_hydro_params)) {
        param_code <- hydro_params_df$param[i]
        param_name <- hydro_params_df$name[i]

        for (scen_code in hydro_scenarios) {
          incProgress(1 / (total_steps + n_hydro_params * n_hydro_scenarios),
            detail = paste("Processing", param_name, "-", gsub("_", " ", toupper(scen_code)))
          )
          ...
          ddf <- try(
            {
              ...
              file_scen_base <- paste0("rdis_", stat, "_", season, "_", scen_code, "_2006-2099_ensmean.nc")
              ...
              if (length(file_hist) == 0 || length(file_scen) == 0) stop("Hydro files not found")

              r_hist <- terra::rast(file_hist)
              r_scen <- terra::rast(file_scen)
              terra::time(r_hist) <- seq(as.Date("1971-01-01"), by = "years", length.out = terra::nlyr(r_hist))
              terra::time(r_scen) <- seq(as.Date("2006-01-01"), by = "years", length.out = terra::nlyr(r_scen))
              r_50 <- c(r_hist, r_scen)

              file_hist_p90 <- gsub("ensmean", "ensp90", file_hist)
              file_scen_p90 <- gsub("ensmean", "ensp90", file_scen)
              file_hist_p10 <- gsub("ensmean", "ensp10", file_hist)
              file_scen_p10 <- gsub("ensmean", "ensp10", file_scen)
              ...
              terra::time(r_hist_p90) <- terra::time(r_hist)
              terra::time(r_scen_p90) <- terra::time(r_scen)
              terra::time(r_hist_p10) <- terra::time(r_hist)
              terra::time(r_scen_p10) <- terra::time(r_scen)
          ...
          hydro_data_list[[plot_id]] <- ddf

          local({
            local_plot_id <- plot_id
            output_id <- paste0("hydro_aoi_plot_", local_plot_id)
            output[[output_id]] <- renderHighchart({
              df_plot <- hydro_data_list[[local_plot_id]]
              if (is.data.frame(df_plot) && nrow(df_plot) > 0 && !all(is.na(df_plot$value))) {
                hydro_params_def <- data.frame(input = "rdis", parm = "River Discharge", unit = "m\u00b3/s", stringsAsFactors = FALSE)
                create_timeseries_chart(data_input = df_plot, param = "rdis", params_def = hydro_params_def)
              } else {
                highchart() %>% hc_title(text = "Data not available")
              }
            })
      ...
      hydro_plots_data(hydro_data_list)

      # --- 6. Hydro Anomaly and Summary Table ---
      hydro_plot_data <- hydro_data_list
      for (i in seq_len(n_hydro_params)) {
        local({
          param_code <- hydro_params_df$param[i]

          all_hydro_data <- lapply(hydro_scenarios, function(scen_code) {
            plot_id <- paste(param_code, scen_code, sep = "_")
            df <- hydro_plot_data[[plot_id]]
            if (is.data.frame(df) && nrow(df) > 0 && !all(is.na(df$value))) {
              df$scenario <- gsub("_", " ", toupper(scen_code))
              return(df)
            }
            return(NULL)
          })
          ...
          if (is.data.frame(combined_hydro_df) && nrow(combined_hydro_df) > 0) {
            for (scen_code in hydro_scenarios) {
              local({
                local_scen_code <- scen_code
                scen_label <- gsub("_", " ", toupper(local_scen_code))
                df_scen <- combined_hydro_df %>% filter(scenario == scen_label)

                historical_mean_scen <- df_scen %>%
                  filter(as.numeric(format(date, "%Y")) >= 1971 & as.numeric(format(date, "%Y")) <= 2005) %>%
                  summarise(mean_value = mean(value, na.rm = TRUE)) %>%
                  pull(mean_value)

                if (!is.na(historical_mean_scen) && historical_mean_scen != 0) {
                  df_scen <- df_scen %>% mutate(
                  ...
                  output_id <- paste("hydro_aoi_anomaly_plot", param_code, local_scen_code, sep = "_")
                  output[[output_id]] <- renderHighchart({
                    if (is.data.frame(df_scen) && nrow(df_scen) > 0 && !all(is.na(df_scen$anomaly))) {
                      hydro_params_def <- data.frame(input = "rdis", parm = "River Discharge", unit = "%", stringsAsFactors = FALSE)
                      create_timeseries_chart(data_input = df_scen, param = "rdis", params_def = hydro_params_def, is_anomaly = TRUE)
                    } else {
                      highchart() %>% hc_title(text = "Anomaly data not available")
                    }
                  })
            ...
            hydro_future_periods <- list("2041-2060" = 2041:2060, "2061-2080" = 2061:2080, "2081-2099" = 2081:2099)
            hydro_summary_list <- list()
            ...
            for (scen_code in hydro_scenarios) {
              ...
              if (is.data.frame(df) && nrow(df) > 0) {
                ...
                hist_mean <- df %>%
                  ...
                  pull(m)
                if (length(hist_mean) > 0 && !is.na(hist_mean) && hist_mean != 0) {
                  for (period_name in names(hydro_future_periods)) {
                    ...
                    fut_mean <- df %>%
                      ...
                      summarise(m = mean(value, na.rm = TRUE)) %>%
                      pull(m)
                    if (length(fut_mean) > 0 && !is.na(fut_mean)) {
                      change <- ((fut_mean - hist_mean) / hist_mean) * 100
                      scen_label <- gsub("_", " ", toupper(scen_code))
                      hydro_summary_list[[length(hydro_summary_list) + 1]] <- data.frame(
                        Scenario = scen_label, Period = period_name,
                        Historical_Mean = round(hist_mean, 1), Future_Mean = round(fut_mean, 1), Change_Pct = round(change, 1)
                      )
                    }
                  }
                }
            ...
            if (length(hydro_summary_list) > 0) {
              hydro_summary_df <- do.call(rbind, hydro_summary_list)
              hydro_summary_wide <- hydro_summary_df %>% tidyr::pivot_wider(names_from = Period, values_from = c("Future_Mean", "Change_Pct"), names_sep = "_")
              output_id <- paste("hydro_summary_table", param_code, sep = "_")
              output[[output_id]] <- renderTable(hydro_summary_wide)
            }
          }
        })
      }
...
output$download_aoi_report <- downloadHandler(
  ...
  content = function(file) {
    withProgress(message = "Generating report, please wait...", {
      ...
      # Clean hydro data for report
      clean_hydro_data <- lapply(hydro_plots_data(), function(x) {
        if (inherits(x, "try-error") || is.null(x) || !is.data.frame(x)) {
          return(data.frame(
            date = as.Date(character()),
            value = numeric(0),
      ...
      local_params <- list(
        ...
        # Hydro params
        hydro_plot_data = clean_hydro_data,
        hydro_params = as.data.frame(hydro_params_df, stringsAsFactors = FALSE),
        hydro_scenarios = as.character(hydro_scenarios)
      )
